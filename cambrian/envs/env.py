...
class MjCambrianEnvConfig(HydraContainerConfig):
    ...
    overlay_mode: str = "debug"  # debug|minimal|off
...
class MjCambrianEnv(ParallelEnv, Env):
    ...
    def __init__(self, config: MjCambrianEnvConfig, name: Optional[str] = None):
        ...
        if renderer_config := self._config.renderer:
            ...
            if getattr(renderer_config, "cinematic", None) is not None:
                self._renderer = MjCambrianCinematicRenderer(renderer_config)
            else:
    ...
    def generate_xml(self) -> MjCambrianXML:
        ...
        if (
            self._config.renderer is not None
            and getattr(self._config.renderer, "scene_xml_overrides", None) is not None
        ):
            xml += MjCambrianXML.from_config(self._config.renderer.scene_xml_overrides)
        ...
        if (
            self._config.renderer is not None
            and getattr(self._config.renderer, "theme", None) is not None
        ):
            xml += MjCambrianXML.from_config(self._generate_theme_xml_overrides())

        return xml

    def _generate_theme_xml_overrides(self) -> List[Dict[str, Any]]:
        theme = getattr(self._config.renderer, "theme", None)
        if theme not in {"deep_sea", "cambrian_shallows"}:
            return []

        theme_palettes = {
            "deep_sea": {
                "goal": {
                    "body_rgba": "0.16 0.35 0.33 1.0",
                    "top_rgba": "0.56 0.90 0.82 1.0",
                    "emission": "1.6",
                    "specular": "0.08",
                    "shininess": "0.22",
                },
                "adversary": {
                    "body_rgba": "0.30 0.22 0.24 1.0",
                    "top_rgba": "0.82 0.48 0.42 1.0",
                    "emission": "1.25",
                    "specular": "0.06",
                    "shininess": "0.18",
                },
            ...
            "cambrian_shallows": {
                "goal": {
                    "body_rgba": "0.34 0.62 0.68 1.0",
                    "top_rgba": "0.84 0.96 0.96 1.0",
                    "emission": "1.3",
                    "specular": "0.10",
                    "shininess": "0.28",
                },
                "adversary": {
                    "body_rgba": "0.58 0.46 0.40 1.0",
                    "top_rgba": "0.94 0.74 0.62 1.0",
                    "emission": "1.05",
                    "specular": "0.08",
                    "shininess": "0.22",
                },
        ...
        for name in self._agents:
            ...
            if name.startswith("goal"):
                palette = theme_palette["goal"]
            elif name.startswith("adversary"):
                palette = theme_palette["adversary"]
                    ...
                    "mujoco": [
                        {
                            "asset": [
                                {
                                    "material": [
                                        {"name": f"{name}_deep_sea_mat"},
                                        {"rgba": palette["body_rgba"]},
                                ...
                                {
                                    "material": [
                                        {"name": f"{name}_deep_sea_top_mat"},
                                        {"rgba": palette["top_rgba"]},
                                        {"emission": palette["emission"]},
                                        {"specular": palette["specular"]},
                                        {"shininess": palette["shininess"]},
                                    ]
                                    ...
                                    "body": [
                                        ...
                                        },
                                        {
                                            "site": [
                                                ...
                                                {
                                                    "material": f"{name}_deep_sea_top_mat"
                                                },
                                                {"rgba": palette["top_rgba"]},
                                            ]
                                        },
    ...
    def render(self) -> RenderFrame:
        ...
        if self._config.add_overlays:
            overlay_mode = self._config.overlay_mode
            renderer_overlay_mode = getattr(
                self._config.renderer, "default_overlay_mode", None
            )
            if overlay_mode == "debug" and renderer_overlay_mode is not None:
                overlay_mode = renderer_overlay_mode
            if self._config.cinematic_mode and overlay_mode == "debug":
                overlay_mode = "off"
            ...
            if overlay_mode == "minimal":
                overlays.extend(
                    ...
                    ).to_viewer_overlays(self._renderer.width, self._renderer.height)
                )
            elif overlay_mode == "debug":
                debug_fields = {}
                for key, value in self._overlays.items():
                    if not isinstance(value, MjCambrianViewerOverlay):
                        debug_fields[key] = value
                    else:
                        overlays.append(value)

                overlays.extend(
                    MjCambrianDebugOverlay(debug_fields).to_viewer_overlays(
                ...
                raise ValueError(
                    f"Unknown overlay mode: {overlay_mode}. "
                    "Expected one of: debug|minimal|off."
                )
