...
DEEP_SEA_THEME_ID: str = "deep_sea"
CAMBRIAN_SHALLOWS_THEME_ID: str = "cambrian_shallows"
THEME_WALL_MATERIALS = {
    DEEP_SEA_THEME_ID: {
        "rgba": "0.20 0.30 0.34 1.0",
        "specular": "0.03",
        "shininess": "0.05",
    },
    CAMBRIAN_SHALLOWS_THEME_ID: {
        "rgba": "0.42 0.54 0.60 1.0",
        "specular": "0.05",
        "shininess": "0.09",
    },
}
...
class MjCambrianMazeEnv(MjCambrianEnv):
    def __init__(self, config: MjCambrianMazeEnvConfig, **kwargs):
        self._config = config
        self._visual_theme = self._resolve_visual_theme(config)
    ...
    @staticmethod
    def _resolve_visual_theme(config: MjCambrianMazeEnvConfig) -> Optional[str]:
        if config.renderer is None:
            return None
        return getattr(config.renderer, "theme", None)
    ...
    def reset(
        ...
        if (renderer := self.renderer) and (viewer := renderer.viewer):
            if viewer.camera.type == mj.mjtCamera.mjCAMERA_FREE:
...
class MjCambrianMaze:
    ...
    def __init__(
        ...
        *,
        ...
        self._config = config
        self._name = name
        self._visual_theme = visual_theme
    ...
    def generate_xml(self) -> MjCambrianXML:
        ...
        if self._visual_theme in {DEEP_SEA_THEME_ID, CAMBRIAN_SHALLOWS_THEME_ID}:
            floor.attrib["material"] = f"floor_mat_{self._visual_theme}_{self._name}"

        return xml

    def _wall_material_overrides(self) -> Dict[str, str]:
        return dict(THEME_WALL_MATERIALS.get(self._visual_theme, {}))
