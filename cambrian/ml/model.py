...
class MjCambrianModel(PPO):
    ...
    @classmethod
    def load_checkpoint(cls, path: Path | str, **kwargs):
        """Load a model from either SB3 zip checkpoints or Cambrian policy folders.

        Supported inputs:
            - `<path>` where `<path>.zip` exists (SB3 full checkpoint)
            - `<path>.zip` (SB3 full checkpoint)
            - directory containing `policy.pt` (Cambrian policy-only checkpoint)
            - direct `policy.pt` path
        """
        path = Path(path)

        # Prefer SB3 full checkpoints when present.
        zip_path = path if path.suffix == ".zip" else path.with_suffix(".zip")
        if zip_path.exists():
            # SB3 accepts loading with the suffix-less base path.
            base_path = path if path.suffix != ".zip" else path.with_suffix("")
            return cls.load(base_path, **kwargs)

        # Fall back to Cambrian policy-only checkpoints.
        if path.is_dir():
            ...
            if policy_path.exists():
                ...
                return model
        elif path.name == "policy.pt" and path.exists():
            model = cls(**kwargs)
            model.load_policy(path.parent)
            return model

        raise FileNotFoundError(
            f"Could not find a checkpoint at {path}. "
            f"Expected either {zip_path} or a policy.pt file."
        )
