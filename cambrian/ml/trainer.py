...
class MjCambrianTrainer:
    ...
    def eval(
        ...
        **callback_kwargs,
        ...
        renderer_config = self._config.eval_env.renderer
        record_source = getattr(renderer_config, "record_source", "scene")
        if record_source in {"agent_vision", "side_by_side"}:
            vision_config = getattr(renderer_config, "vision", {}) or {}
            vision_recorder = None
            if record:
                vision_recorder = MjCambrianVisionRecorder(
            ...
            def vision_step_callback(env):
                if vision_recorder is not None:
                    ...
                    if record_source == "agent_vision":
                        ...
                        vision_recorder.capture_frame(frame)
                step_callback = callback_kwargs.get("step_callback")
                if step_callback is not None:
                    return step_callback(env)
                return True
            ...
            evaluate_policy(
                ...
                n_runs,
                record_kwargs=None,
                step_callback=vision_step_callback,
                done_callback=callback_kwargs.get("done_callback", lambda _: True),
            )
            if vision_recorder is not None:
                vision_recorder.save(self._config.expdir / filename)
        else:
            record_kwargs = dict(
