...
class MjCambrianOpticsEye(MjCambrianEye):
    ...
    def step(
        ...
        depth = self._sanitize_depth_map(depth)
    ...
    def _sanitize_depth_map(self, depth: torch.Tensor) -> torch.Tensor:
        """Remove sky-depth values and clamp to the valid optical working range."""
        max_depth = torch.max(depth)
        valid = depth[depth < max_depth]
        min_depth = torch.tensor(
            5 * max(self.config.focal), device=depth.device, dtype=depth.dtype
        )
        if valid.numel() > 0:
            min_depth = torch.clamp(torch.min(valid), min=min_depth)
        return torch.where(depth < max_depth, depth, min_depth).clamp(min=min_depth)

    def _sensor_pitch(self) -> float:
        sensor_h, sensor_w = self._config.sensorsize
        res_h, res_w = self._config.resolution
        pitch_h = sensor_h / max(res_h, 1)
        pitch_w = sensor_w / max(res_w, 1)
        return float((pitch_h + pitch_w) / 2.0)

    @staticmethod
    def _image_distance_from_object_distance(
        object_distance: torch.Tensor, focal_length: float
    ) -> torch.Tensor:
        eps = torch.finfo(object_distance.dtype).eps
        return focal_length * object_distance / torch.clamp(
            object_distance - focal_length, min=eps
        )

    def _circle_of_confusion_pixels(
        self,
        ...
        *,
        ...
        """Thin-lens circle-of-confusion in pixel units."""
        focal_length = float(focal_length or sum(self._config.focal) / 2.0)
        f_number = float(f_number or self._config.f_stop)
        aperture_diameter = focal_length / max(f_number, 1e-6)

        object_distance = depth_map.clamp(min=focal_length * 1.001)
        focus_tensor = torch.full_like(object_distance, float(focus_distance))
        image_distance = self._image_distance_from_object_distance(
            object_distance, focal_length
        )
        focus_image_distance = self._image_distance_from_object_distance(
            focus_tensor, focal_length
        )

        coc_sensor = aperture_diameter * torch.abs(
            image_distance - focus_image_distance
        ) / torch.clamp(image_distance, min=1e-6)
        return coc_sensor / max(self._sensor_pitch(), 1e-6)
