...
class MjCambrianRendererConfig(HydraContainerConfig):
    ...
    scene_xml_overrides: Optional[Any] = None
    camera_name: Optional[str] = None
    default_overlay_mode: Optional[str] = None
    record_source: str = "scene"
...
class MjCambrianRenderer:
    ...
    def reset(
        ...
        self._configure_fixed_camera_from_name()

        return self.render(resetting=True)

    def _configure_fixed_camera_from_name(self) -> None:
        camera_name = getattr(self._config, "camera_name", None)
        if not camera_name:
            return

        fixedcamid = self._spec.get_camera_id(camera_name)
        if fixedcamid < 0:
            available = [
                self._spec.get_camera_name(i)
                for i in range(self._spec.model.ncam)
            ]
            raise ValueError(
                f"Renderer camera_name '{camera_name}' not found. "
                f"Available cameras: {available}"
            )

        self._viewer.camera.type = mj.mjtCamera.mjCAMERA_FIXED
        self._viewer.camera.fixedcamid = fixedcamid
