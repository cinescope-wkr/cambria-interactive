...
@dataclass(slots=True)
class MjCambrianDebugOverlay:
    """Structured debug overlay payload for lab-style diagnostics."""

    fields: Dict[str, Any]

    def to_viewer_overlays(
        self, width: int, height: int
    ) -> List[MjCambrianViewerOverlay]:
        cursor = MjCambrianCursor(
            width, height, position=MjCambrianCursor.Position.TOP_LEFT
        )
        overlays: List[MjCambrianViewerOverlay] = []
        for key, value in self.fields.items():
            overlay = MjCambrianViewerOverlay.create_text_overlay(f"{key}: {value}")
...
@dataclass(slots=True)
class MjCambrianMinimalOverlay:
    """Minimal cinematic overlay: only exp name and step on bottom edge."""
    ...
    def to_viewer_overlays(
        ...
        # MuJoCo text overlay API does not expose granular font sizes. Keep content
        # minimal and place it away from the center for cinematic framing.
        cursor = MjCambrianCursor(
            width, height, position=MjCambrianCursor.Position.BOTTOM_LEFT
        )
        overlays = [
            MjCambrianViewerOverlay.create_text_overlay(f"{self.exp_name}"),
            MjCambrianViewerOverlay.create_text_overlay(f"step {self.step}"),
        ]
